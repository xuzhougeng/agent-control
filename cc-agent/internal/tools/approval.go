...
var dangerousMatchers = []struct {
	...
	{regexp.MustCompile(`(?i)\bremove-item\b`), "Remove-Item"},
	{regexp.MustCompile(`(?i)\b(del|erase|rmdir|rd)\b\s+`), "Windows delete"},
	{regexp.MustCompile(`(?i)\b(stop-service|restart-service)\b`), "service stop/restart"},
	{regexp.MustCompile(`(?i)\bsc(\.exe)?\s+(stop|delete|config)\b`), "service stop/config"},
	{regexp.MustCompile(`(?i)\bnet(\.exe)?\s+stop\b`), "service stop"},
	{regexp.MustCompile(`(?i)\bformat(\.com|\.exe)?\b`), "format"},
